...
|---|---|
| `check` | Runs `fmt`, `clippy`, `test`, `check-deps`, `check-no-game-ids`, `check-manifests`, token freshness check, `check-no-raw-colors`, then `cargo deny check`, in that order, stopping at the first failure. The one command a PR needs to be confident in; see AGENTS.md §5. |
...
| `pack-assets <game>` | 3 | Builds, hashes, and writes a pack manifest (planned Phase 3) |
