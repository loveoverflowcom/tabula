...
use tabula_game_tiles::{rules::first_legal_placement, Command, Config, State, Status, TilesRules};
...
/// The placement a "first legal in canonical order" player would make now.
...
/// Play up to `turns` placements from the opening position, returning the
/// reached state and the script that reached it.
pub fn drive(
    seed: &MatchSeed,
    seats: u8,
    cfg: Config,
...
/// A script that plays the match to its terminal state.
pub fn full_script(seed: &MatchSeed, seats: u8, cfg: Config) -> Vec<Input<Command>> {
    // The bag holds 71 tiles; no match can need more inputs than that.
